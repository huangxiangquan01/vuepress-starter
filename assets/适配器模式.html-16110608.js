import{_ as n,o as s,c as a,a as t}from"./app-ec3e5753.js";const p={},e=t(`<h3 id="一、什么是适配器模式" tabindex="-1"><a class="header-anchor" href="#一、什么是适配器模式" aria-hidden="true">#</a> 一、什么是适配器模式：</h3><p>​ 适配器模式主要用于将一个类的接口转化成客户端希望的目标类格式，使得原本不兼容的类可以在一起工作，将目标类和适配者类解耦；同时也符合“开闭原则”，可以在不修改原代码的基础上增加新的适配器类；将具体的实现封装在适配者类中，对于客户端类来说是透明的，而且提高了适配者的复用性，但是缺点在于更换适配器的实现过程比较复杂。</p><p>​ 所以，适配器模式比较适合以下场景：</p><ul><li>系统需要使用现有的类，而这些类的接口不符合系统的接口。</li><li>使用第三方组件，组件接口定义和自己定义的不同，不希望修改自己的接口，但是要使用第三方组件接口的功能</li></ul><h3 id="二、适配器模式的三种实现方式" tabindex="-1"><a class="header-anchor" href="#二、适配器模式的三种实现方式" aria-hidden="true">#</a> 二、适配器模式的三种实现方式：</h3><p>​ 适配器模式主要分成三类：类的适配器模式、对象的适配器模式、接口的适配器模式。</p><p><strong>1、类的适配器模式：</strong></p><ul><li>目标接口（Target）：客户所期待的接口。目标可以是具体的或抽象的类，也可以是接口。</li><li>需要适配的类（Adaptee）：需要适配的类或适配者类。</li><li>适配器（Adapter）：通过包装一个需要适配的对象，把原接口转换成目标接口。</li></ul><div class="language-java line-numbers-mode" data-ext="java"><pre class="language-java"><code><span class="token comment">// 已存在的、具有特殊功能、但不符合我们既有的标准接口的类</span>
